...
import { faInfoCircle, faCalendarAlt, faMapMarkerAlt, faImages, faComments, faStar, } from "@fortawesome/free-solid-svg-icons";
...
const ToursDetail = () => {
  ...
  const handleBooking = async (e) => {
    ...
    if (!formData.name || !formData.email || !formData.confirmEmail || !formData.date) {
      Swal.fire({
        ...
        text: "Please fill all required fields (Name, Email, Confirm Email, Date).",
      });
      return;
    }
    ...
    if (formData.tickets && isNaN(formData.tickets)) {
      Swal.fire({
        ...
        text: "Number of tickets must be a number.",
      });
      return;
    }
    ...
    try {
      ...
      Swal.fire({
        ...
        text: `Your trip is booked for ${formattedDate}`,
        icon: "success",
      });
  ...
  return (
    <Box sx={{ bgcolor: "#fff", minHeight: "100vh" }}>
      <Box sx={{ position: "relative", height: "70vh", backgroundImage: `url(${tour.img})`, backgroundSize: "cover", backgroundPosition: "center", }} >
        ...
        <Box sx={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", color: "#fff", textAlign: "center" }} >
          <div className="relative text-center z-10 px-4">
            <div className="mb-6">
              <Link to="/tour" className="inline-block bg-white/20 text-white py-2 px-4 rounded-md backdrop-blur-sm hover:bg-white/30 transition" > ← Back </Link>
            </div>
          </div>
          <Typography variant="h6" sx={{ fontFamily: "cursive", mb: 1 }}>
            Amazing Tour
          </Typography>
          <Typography variant="h2" sx={{ fontWeight: 700, textTransform: "capitalize", fontSize: { xs: "2rem", md: "4rem" }, }} >
            {tour.title}
          </Typography>
        </Box>
      </Box>
        ...
        {[
          ...
          <Button
            ...
            sx={{
              ...
              fontWeight: 600,
              borderBottom:
                activeTab === tab.id ? "2px solid #00bfa6" : "2px solid transparent",
              borderRadius: 0,
              "&:hover": { color: "#00bfa6" },
            }}
          >
      ...
      <Box className="grid grid-cols-1 md:grid-cols-3 gap-10 px-8 md:px-20 py-16">
        <Box className="md:col-span-2">
          {activeTab === "information" && (
            <>
              ...
              <Typography variant="h6" sx={{ color: "#00bfa6", fontWeight: 600 }}>
                ${tour.price}
              </Typography>
        ...
        <Box
          sx={{ bgcolor: "#2ec4b6", p: 4, borderRadius: 2, color: "#fff", boxShadow: 3, height: "fit-content", }} component="form" onSubmit={handleBooking} >
          <Typography variant="h5" sx={{ fontWeight: 700 }}>
            Book This Tour
          </Typography>

          <Typography sx={{ mb: 3 }}>
            Arrange your trip in advance – book now!
          </Typography>
          ...
          {[
            ...
            <TextField key={i} name={name} placeholder={label} variant="outlined" fullWidth value={formData[name]} onChange={handleInputChange} sx={{ bgcolor: "#3ddad7", mb: 2, borderRadius: 1, "& .MuiOutlinedInput-root": { "& fieldset": { border: "none" },}, input: { color: "#fff" }, }} />
          ))}

          <TextField name="date" type="date" label="Select Date *"
            InputLabelProps={{ shrink: true }} inputProps={{ min: today }} variant="outlined" fullWidth value={formData.date} onChange={handleInputChange} sx={{ bgcolor: "#3ddad7", mb: 2, borderRadius: 1, "& .MuiOutlinedInput-root": { "& fieldset": { border: "none" } }, input: { color: "#fff" },}} />

          <Button type="submit" variant="contained" fullWidth sx={{ backgroundColor: "#fff", color:"#2ec4b6", fontWeight: "bold", "&:hover": { backgroundColor: "#f2f2f2" }, }}>
            BOOK NOW
          </Button>
        </Box>
      </Box>
